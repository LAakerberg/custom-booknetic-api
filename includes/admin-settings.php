...
function booknetic_api_admin_page() {
    ...
    if (isset($_POST['booknetic_save_keys'])) {
        ...
        foreach ($partners as $i => $api_key_name) {
            ...
            $enabled = isset($enabled_keys[$api_key_name]) ? 1 : 0;
            $methods = isset($method_keys[$api_key_name]) ? implode(',', array_map('sanitize_text_field', $method_keys[$api_key_name])) : '';

            $existing = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table WHERE api_key_name = %s", $api_key_name));

            if ($existing) {
                // Check if enabled/disabled status changed
    ...
    <td><input type="checkbox" name="enabled[<?php echo esc_attr($api_key_name); ?>]" <?php checked($row['enabled']); ?> /></td>
    <td>
        <?php foreach (['GET', 'POST', 'PUT', 'DELETE'] as $method): ?>
            <label><input type="checkbox" name="methods[<?php echo esc_attr($api_key_name); ?>][]" value="<?php echo $method; ?>" <?php checked(in_array($method, explode(',', $row['methods']))); ?> /> <?php echo $method; ?></label><br>
        <?php endforeach; ?>
    </td>
    <td>
        <?php
